...
## Last-Updated: 2021-12-21T10:57:17+0100
################
...
predictionbounds <- function(x,n=100){
    fo <- (1+sqrt(1-(1-x)^(4/3)))/2
    rbind(c((1+x)*n/2,sqrt((1-x)*n)),
          c(fo*n, 2*sqrt((1-fo)*fo*n)))
}
...
dirname <- 'Ing_posteriorA_-V13-D678-K75-I1024'
frequenciesfile <- '_frequencies-RIng_posteriorA_3-V13-D678-K75-I1024.rds'
...
## pdff(paste0(dirname,'/','data_histograms_given_AD'))
## for(acov in otherCovs){
##     agrid <- grids[[acov]]
##     histo <- list()
##     ymax <- -Inf
##     for(i in 1:2){
##         histo[[i]] <- thist(c(data.matrix(alldata[get(maincov)==(i-1),..acov])),n=if(acov %in% realCovs){seq(min(agrid),max(agrid),length.out=32)}else{'i'})
...
##         tplot(x=histo[[i]]$mids,y=-freqs/maxf*ymax,col=i)
##     }
...
singlemi <- t(sapply(otherCovs, function(acov){
    ...
    readRDS(paste0(dirname,'/probsingle_',acov,'.rds'))
    probjoint <- #rowMeans(samplesF(Y=YX[,c(maincov,acov),drop=F], parmList=parmList), na.rm=T)
    readRDS(paste0(dirname,'/probjoint_',acov,'.rds'))
...
dropmis <- t(sapply(otherCovs, function(acov){
    ...
    readRDS(paste0(dirname,'/probjointminus_',acov,'.rds'))
    probsingle <- #rowMeans(samplesF(Y=YX[,setdiff(otherCovs,acov),drop=F], parmList=parmList), na.rm=T)
    readRDS(paste0(dirname,'/probsingleminus_',acov,'.rds'))
    c(mean(log2(probjoint/(probY*probsingle)), na.rm=T),
      sd(log2(probjoint/(probY*probsingle)), na.rm=T)/errcorr)
}))
