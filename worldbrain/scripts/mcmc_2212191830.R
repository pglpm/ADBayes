...
baseversion <- '_inference1' # *** ## Base name of output directory
maxstages <- Inf
ncores <- 1
...
initsFunction <- function(){
    c(
        ...
        if(len$R > 0){list( # real variate
                     ...
                     Rvarscaleindex = sample(1:nrvarscales,len$R,replace=TRUE),
                     ## Rvarscaleindex = rep(ceiling(nrvarscales/2),len$R),
        ...
        if(len$O > 0){list( # doubly-bounded variate
                     ...
                     Ovarscaleindex = sample(1:novarscales,len$O,replace=TRUE),
                     ## Ovarscaleindex = rep(ceiling(novarscales/2),len$O),
        ...
        if(len$D > 0){list( # doubly-bounded variate
                     ...
                     Dvarscaleindex = sample(1:ndvarscales,len$D,replace=TRUE),
                     ## Dvarscaleindex = rep(ceiling(ndvarscales/2),len$D),
        ...
        if(len$I > 0){list( # integer ordinal variate
                     ...
                     Ivarscaleindex = sample(1:nivarscales,len$I,replace=TRUE),
                     ## Ivarscaleindex = rep(ceiling(nivarscales/2),len$I),
