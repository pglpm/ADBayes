...
## Last-Updated: 2022-12-26T16:46:29+0100
#########################################
...
baseversion <- '_inferencepi' # Base name of output directory
...
testIAT <- TRUE
nautocorr <- 4 # multiple of IAT for acceptable number of burn-in samples
...
while(continue){
    ...
    lli <- colSums(log(samplesFDistribution(Y=data.matrix(data0[,..predictors]), X=data.matrix(data0[,..predictands]), mcsamples=mcsamples, varinfo=varinfo, jacobian=FALSE)), na.rm=T) #- sum(log(invjacobian(data.matrix(data0[,..predictors]), varinfo)), na.rm=T)
        ...
        #########################################
        if(testIAT){
            if(niterfinal < nautocorr * ceiling(max(diagnIAT))){
                niterfinal <- nautocorr * ceiling(max(diagnIAT))
                cat(paste0('Number of iterations too small. Increasing to ', niterfinal),'\n')
                nitertot <- nitertot + niter
                stage <- stage + 1L
            }else{
                print(paste0('Number of iterations is ', signif(niterfinal/max(diagnIAT),2), ' times the autocorrelation'))
                testIAT <- FALSE
            }
        }else{
